...
class Metadata(object):
    ...
    # TODO something like title probably isn't relevant for bulk import, but
    # something like setting a UUID might be..
    keywords = []
    credit = ""
    description = ""
    copyright = ""
    ...
    # Long form description of Subject and related subject data in a Natural
    # Language caption
...
class Photo(object):
        ...
        try:
            ...
            if md.has_tag(Metadata.TAG_EXIF_DATETIME):
                ...
                try:
                    ...
                    click.echo("Unable to parse the created date " +
                               createdDate + " for " + self.filename)
                    click.echo(ex)
    ...
    def save(self, destinationPath, metadata):
        ...
        # Ignore existing photos, even if it's a broken symlink (for those of
        # us using git annex)
        if os.path.lexists(destinationFullPath):
            click.echo("Photo " + self.filename +
        ...
        os.chmod(destinationFullPath, mode=(stat.S_IRUSR |
...
class PhotoImport(object):
    ...
    def processPhotos(self):
        for photo in self.photos:
            if photo.getCreatedDate() == None:
                ...
                click.echo("Photo " + photo.filename +
                           " does not have a created date. Skipping..")
                continue
    ...
    def createDirectoryStructure(self, photo):
        ...
        destinationAbsPath = os.path.join(os.path.join(os.path.join(
            PhotoImport.absolutePath(self.destination), folderYear), folderMonth), folderDay)
        try:
    ...
    def getPhotos(source):
        ...
        for (dirpath, dirnames, filenames) in os.walk(source):
            for filename in filenames:
                absfilename = PhotoImport.absolutePath(
